...
FIELD_CONTEXTS: dict[tuple[str, str], dict] = {
	("Server Script", "script"): {
		"system_prompt": (
			...
			"Do NOT import modules — only the whitelisted namespace is available.\n"
	...
	("Notification", "condition"): {
		"system_prompt": (
			...
			" evaluated via `frappe.safe_eval`.\n\n"
			...
			"Do NOT use `import`, assignments (`=`), `def`, `for`, `if/else` statements,"
			" or any block syntax — only an expression.\n"
		...
		"variants": {
			"Python": {
				"system_prompt": (
					...
					" executed via `safe_exec`. Restricted to System Manager / Administrator"
					" users.\n\n"
