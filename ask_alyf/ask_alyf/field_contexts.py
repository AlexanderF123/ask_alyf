...
FIELD_CONTEXTS: dict[tuple[str, str], dict] = {
	...
	("Ask ALYF Skill", "description"): {
		"system_prompt": (
			...
			"WHAT YOU CANNOT SEE\n"
			"You observe finished records only. A source document leaves no trace of what it"
			" looked like before someone entered it, so merged lines, a value read off a scan,"
			" and any judgement about which lines deserve their own row are all invisible to"
			" you. Never assert a rule about how source documents map to records. If the"
			" current field value states such a rule, keep it as written.\n\n"
			"OUTPUT\n"
